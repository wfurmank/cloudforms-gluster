...
# Checking if user owns the volume
...
# Stopping volume
stop_output = `#{curl}/api/clusters/#{clid}/glustervolumes/#{volid}/stop -d "<action/>" 2>/dev/null`

# Deleting volume
delete_output = `#{curl}/api/clusters/#{clid}/glustervolumes/#{volid} -XDELETE 2>/dev/null`
...
if `#{curl}/api/clusters/#{clid}/glustervolumes/#{volid} 2>/dev/null`.include? volid
